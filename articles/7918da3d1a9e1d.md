---
title: "MagicOnion + MessagePack + YetAnotherHttpHandler でリアルタイム通信を行う手順"
emoji: "🕌"
...
MagicOnion + MessagePack + YetAnotherHttpHandler でリアルタイム通信を行う手順を書きました。Unityクライアントの IL2CPP ビルドまでやります。

Unity向けの gRPC クライアントとしてサポートの切れた Grpc.Core 実装を使わざるを得ない状況だったのですが、Cysharp さんから grpc-dotnet 実装の YetAnotherHttpHandler が公開されたので、2024年最新版の MagicOnion と MessagePack とを組み合わせてリアルタイム通信をやってみたという記事です。

Unity の gRPC クライアント事情についてはこちらが参考になります。[Unityでもgrpc-dotnetを使ったgRPCがしたい - Activ8 Tech Blog](https://synamon.hatenablog.com/entry/grpc-dotnet-unity)
...
- C++ コンパイラと Windows SDK のインストール
  - Visual Studio Installerを起動して 変更 -> C++ によるデスクトップ開発 を選択しインストールすることができます。
...
また、Shared プロジェクトはサーバー側では .NET8 環境、Unity クライアント側では Unity のコンパイラ環境（C#9.0 まで対応）で実行されるので注意が必要です。

そのため Shared プロジェクトに関しては、以下のように `LangVersion`を`9.0`で指定します。Server プロジェクトは .NET8 なので C#12 まで使えますが、その構文で Shared プロジェクトの中身を書くと Unity環境でコンパイルエラーが出る可能性があります。
...
最後に 適当な GitHub リポジトリに push しておきます。
```
...
Packagesフォルダ配下の`manifest.json`内の `scopedRegistries`として以下を追加します。
...
## Grpc.NET クライアントを使うように Scripting Define Synbols を追加
Edit -> Projectsettings -> Player -> Script Compilationで Scripting Define Synbolsから`USE_GRPC_NET_CLIENT_ONLY`と`USE_GRPC_NET_CLIENT`を追加し、Apply
...
上記の UI の各ボタンと機能を紐づけるようなコードを書きます。
...
そのため、IL2CPP ではリフレクションのために必要な全てのコードを事前に生成しておく必要があります。

MagicOnion や MessagePack には上記を解決するコード生成ツールが提供されているのでそれを利用します。
...
![Alt text](/images/msgpack.png)

### Resolver の登録

実行時に、上記で生成したコード（リフレクション用 Resolver）をStaticなインスタンスに登録します。
...
また、このマテリアルは、`GameObject.CreatePrimitive(PrimitiveType.Cube)` で生成される Cube にアタッチされます。
...
File -> Build Settings -> Player Settings -> Player から
...
# 動かす
magiconion-sample-server の Server プロジェクトを Editor から直接実行するなり、ビルドしてから実行ファイルを実行するなりでサーバーを起動します。
