...
以下のようにプロジェクトファイルに MagicOnion.Server 5.1.8 と Shared プロジェクトを追加します。
...
必要なライブラリをGUIでポチポチと追加するか Assets/package.config に以下を記述するかをしてライブラリを追加します。

さらに MessagePack 2.5.140 は`Microsoft.NET.StringTools`のv1.0.0に依存しているので追加しておきます。
